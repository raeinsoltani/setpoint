...
class Run:
    ...
    def contract(self) -> str:
        """How load was delivered to the fleet.

        Runs before 2026-08-13 predate the field entirely, and they all share the
        defect in lab-notebook §11.13: k6's default keep-alive pinned every connection
        to one backend pod, so 3-4 pods served no matter how many were running. They
        are named rather than left blank because a missing value is exactly the case
        that must not be read as "same as the others".
        """
        return self.meta.get("load_contract") or "shared-connections-pre-v2"
...
def contract_banner(latest: Dict[Tuple[str, str], Run]) -> List[str]:
    """Warn, in the document itself, when the tables below mix load contracts.

    A mixed table is the specific way §11.13 could do lasting damage: the numbers all
    look equally trustworthy, and nothing on the row says one of them was produced by a
    harness that only reached a third of the fleet. This does not refuse to emit the
    tables — re-measurement is necessarily incremental, so refusing would make the
    tool unusable exactly when it is needed — but the mixture is never left implicit.
    """
    by_contract: Dict[str, List[str]] = {}
    for (pattern, arm), run in sorted(latest.items()):
        by_contract.setdefault(run.contract, []).append(f"{pattern}/{arm}")
    ...
    # Enumerate the small groups and count the large one. During a re-measurement the
    # useful question is "which rows have been redone", and a 33-item list of the ones
    # that have not buries it.
    for contract in sorted(by_contract, key=lambda c: len(by_contract[c])):
        members = by_contract[contract]
        if len(members) <= 10:
            shown = ", ".join(f"`{m}`" for m in members)
            lines.append(f"| `{contract}` | **{len(members)}** — {shown} |")
        else:
            lines.append(f"| `{contract}` | **{len(members)}** — everything not listed above |")
    return lines


def write_summary(by_pattern: Dict[str, List[Metrics]], excluded: List[Run], path: str,
                  repeats: Optional[Dict[Tuple[str, str], List[Metrics]]] = None,
                  latest: Optional[Dict[Tuple[str, str], Run]] = None) -> None:
    lines = [
    ...
    lines += contract_banner(latest or {})
    lines += [
        "",
        "## Headline",
        "",
...
def main() -> int:
    ...
    contracts = sorted({r.contract for r in kept})
    if len(contracts) > 1:
        print(f"  WARNING: kept runs span {len(contracts)} load contracts: "
              f"{', '.join(contracts)}", file=sys.stderr)
        print("  see the banner in summary.md; latency and hpa-custom are not "
              "comparable across them (§11.13, §11.15)", file=sys.stderr)
    ...
    write_summary(by_pattern, excluded, sp, repeats, latest)
