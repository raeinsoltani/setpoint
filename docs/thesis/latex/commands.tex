...
% اجازه‌ی شکستن نشانی‌های اینترنتی بلند در فهرست مراجع. بدون این بسته، نشانی
% بلند بنیاد CNCF حدود ۱۲۰ نقطه از حاشیه بیرون می‌زند.
\usepackage{xurl}
...
% در حالت غیرفعال، ارقامِ فرمول‌ها لاتین می‌مانند \mdash{} که برای متنی با این حجم از
...
%-------------------------------------------------------------------------------
% چکیده‌های چندپاراگرافی.
%
% کلاس، \fa-abstract و \en-abstract را با \def تعریف می‌کند و نه با \long\def، پس
% هر پاراگراف تازه در متن چکیده خطای «Paragraph ended before \fa was complete»
% می‌دهد؛ چکیده‌ی نمونه‌ی الگو یک‌پاراگرافی است و به این مشکل نمی‌خورد.
% تعریف زیر تنها همان دو ماکرو را long می‌کند و رفتار دیگری را تغییر نمی‌دهد.
%-------------------------------------------------------------------------------
\long\def\fa-abstract#1{\long\gdef\ffa-abstract{#1}}
\long\def\en-abstract#1{\long\gdef\een-abstract{#1}}

\renewcommand\proofname{\textbf{برهان}}
% زی‌پرشین عنوان‌های فارسی را در \begin{document} تنظیم می‌کند، پس تغییر نام
% کتاب‌نامه باید به همان لحظه موکول شود وگرنه بازنویسی می‌شود.
\AtBeginDocument{\renewcommand{\bibname}{منابع و مراجع}}

%-------------------------------------------------------------------------------
% نویسه‌هایی که قلم B Nazanin ندارد.
%
% این قلم فقط حروف فارسی و ارقام فارسی/لاتین دارد. خط تیره‌ی بلند، جداکننده‌ی
% اعشار، نشانه‌ی درصد و چند نویسه‌ی دیگر در آن نیستند و به‌صورت مربع توخالی
% چاپ می‌شوند. ماکروی زیر خط تیره‌ی بلند را از قلم لاتین می‌گیرد؛ بقیه‌ی موارد
% در خود متن با معادل موجود جایگزین شده‌اند (اعشار با «/» و هزارگان با «،»،
% مطابق رسم‌الخط رایج فارسی).
\DeclareRobustCommand{\mdash}{\lr{\textemdash}}
%-------------------------------------------------------------------------------
...
% تأکید در متن فارسی.
%
% قلم B Nazanin شکل ایتالیک ندارد، پس \emph به‌طور پیش‌فرض به همان شکل عادی
% برمی‌گردد و تأکید عملاً دیده نمی‌شود (هشدار
% «Font shape TU/BNazanin/m/it undefined»). قرارداد متعارف در حروف‌چینی فارسی،
% تأکید با سیاه‌کردن است.
\DeclareTextFontCommand{\emph}{\bfseries}

% کشش اضطراری: شناسه‌های لاتین بلند (نام آزمون‌ها، نام سنجه‌ها) شکستنی نیستند و
% بدون این تنظیم از حاشیه بیرون می‌زنند. با این مقدار، TeX سطر را کمی باز می‌کند
% به‌جای آنکه سرریز کند.
